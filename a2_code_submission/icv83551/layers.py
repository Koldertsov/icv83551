...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(dout * x_norm, axis=0)
    dx_hat = dout * gamma
    dx = (1.0 / N) * (gamma / np.sqrt(sample_var + eps)) * (
        N * dout * 1.0 - np.sum(dout, axis=0) - x_norm * np.sum(dout * x_norm, axis=0)
    )
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    dx = (1.0 / D) * std_inv * (
        D * dx_norm - np.sum(dx_norm, axis=1, keepdims=True) - 
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out_reshaped = out_flat.reshape(N, H, W, C)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_reshaped = dx_flat.reshape(N, H, W, C)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_flat = x_grouped.reshape(N * G, -1)
    ...
    out = gamma * x_norm + beta
    cache = (x_flat, x_norm, mean, var, G, gamma, beta, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    x_flat, x_norm, mean, var, G, gamma, beta, eps = cache
    ...
    x_norm_flat = x_norm.reshape(N, G, C // G, H, W).reshape(N * G, D)
    std_inv = 1.0 / np.sqrt(var + eps)
    
    term1 = D * dx_norm_flat
    term2 = np.sum(dx_norm_flat, axis=1, keepdims=True)
    term3 = x_norm_flat * np.sum(dx_norm_flat * x_norm_flat, axis=1, keepdims=True)
