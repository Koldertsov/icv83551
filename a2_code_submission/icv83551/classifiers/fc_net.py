...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Final Affine Layer (No normalization, ReLU, or dropout)
        w_final = self.params['W' + str(self.num_layers)]
        b_final = self.params['b' + str(self.num_layers)]
        scores, cache_final = affine_forward(current_input, w_final, b_final)
        caches[self.num_layers] = cache_final
        ...
        # 1. Final Affine Layer Backward
        d_prev, dw_final, db_final = affine_backward(dscores, caches[self.num_layers])
        ...
        # 2. Hidden Layers Backward
        for i in range(self.num_layers - 1, 0, -1):
            fc_cache, norm_cache, relu_cache, drop_cache = caches[i]
            
            # Dropout Backward (New addition - must be first in the hidden layer sequence)
            if self.use_dropout:
                d_prev = dropout_backward(d_prev, drop_cache)
            
            # Activation Backward
            d_prev = relu_backward(d_prev, relu_cache)
            
            # Normalization Backward
            if self.normalization == 'batchnorm':
                d_prev, dgamma, dbeta = batchnorm_backward(d_prev, norm_cache)
                grads['gamma' + str(i)], grads['beta' + str(i)] = dgamma, dbeta
            elif self.normalization == 'layernorm':
                d_prev, dgamma, dbeta = layernorm_backward(d_prev, norm_cache)
                grads['gamma' + str(i)], grads['beta' + str(i)] = dgamma, dbeta
                
            # Affine Backward
            d_prev, dw, db = affine_backward(d_prev, fc_cache)
