...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        x_2 = np.sum(np.square(X), axis=1).reshape(num_test, 1)
        train_2 = np.sum(np.square(self.X_train), axis=1).reshape(1, num_train)
        two_dot = 2 * np.dot(X, self.X_train.T)
        
        dists = np.sqrt(x_2 + train_2 - two_dot)
